...
if osp == 'windows' then
    quotes_path = os.getenv('AppData')..'\\mpv\\writing-quotes'
else
    quotes_path = os.getenv('HOME')..'/.config/mpv/writing-quotes'
end
                    ...
                    if (marker >= element.slider.min.value) and (marker <= element.slider.max.value) then
                        ...
                        if (slider_lo.gap > 5) then -- draw triangles
                            ...
                            if (slider_lo.nibbles_top) then
                                static_ass:move_to(s - 2, slider_lo.gap - 5)
                                static_ass:line_to(s + 2, slider_lo.gap - 5)
                                static_ass:line_to(s, slider_lo.gap - 1)
                            end
                            --bottom
                            if (slider_lo.nibbles_bottom) then
                                static_ass:move_to(s - 2, elem_geo.h - slider_lo.gap + 5)
                                static_ass:line_to(s, elem_geo.h - slider_lo.gap + 1)
                                static_ass:line_to(s + 2, elem_geo.h - slider_lo.gap + 5)
                            end
