local settings = {
  ...
  key_sortplaylist = "",
  key_sortplaylistbyname="",
  key_sortplaylistbydatedesc="",
  key_shuffleplaylist = "",
  key_reverseplaylist = "",
  key_loadfiles = "",
  key_saveplaylist = "",
...
function sortplaylistby(sort_mode_id)
  sort_mode = 1
  for mode, sort_data in pairs(sort_modes) do
    if sort_data.id == sort_mode_id then
      sort_mode = mode
    end
  end
  sortplaylist()
...
bind_keys(settings.key_playfirst, "playfirst", function()
  local playlist = mp.get_property_native('playlist')
  if #playlist < 2 then
    return
  else
...
bind_keys(settings.key_playlast, "playlast", function()
  local playlist = mp.get_property_native('playlist')
  if #playlist < 2 then
    return
  else
...
bind_keys(settings.key_sortplaylistbyname, "sortplaylistbyname", function()
  sortplaylistby("name-asc")
end)
bind_keys(settings.key_sortplaylistbydatedesc, "sortplaylistbydatedesc", function()
  sortplaylistby("date-desc")
  mp.set_property('playlist-pos', 0)
end)
