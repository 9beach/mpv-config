...
local o = {
    ...
    mac_copy = 'LC_CTYPE=UTF-8 pbcopy', --copy command that will be used in MAC. OR write a different command
...
function copy()
    if filePath ~= nil then
        ...
        mp.commandv("script-message", "copy-quote")
    end
end
...
function copy_specific(action)
    ...
    if filePath == nil then
        if o.osd_messages == true then
            mp.osd_message('Failed to Copy: No Video Found')
        end
        ...
        if action == 'timestamp' then
            ...
            if o.osd_messages == true then
                mp.osd_message("Timestamp copied")
            end
        ...
        if action == 'path&timestamp' then
            ...
            local video_time = mp.get_property_number('time-pos')
            if o.osd_messages == true then
                mp.osd_message("Path and timestamp copied")
            end
...
function trigger_paste_action(action)
    ...
    if action == 'load-file' then
        ...
        if o.osd_messages == true then
            if clip_time ~= nil then
                mp.osd_message("Clip file and time pasted")
            else
                mp.osd_message("Clip file pasted")
            end
        end
    ...
    if action == 'load-subtitle' then
        if o.osd_messages == true then
            mp.osd_message("Subtitle pasted")
        end
...
mp.register_script_message("copy-to-clipboard", function(name, text)
    set_clipboard(text)
    if o.osd_messages == true then
        msg.info(name.." copied: "..text)
        mp.osd_message(name.." copied")
    end
end)
