...
REM If your machine has NVIDIA GPU installed, run `install.bat nvidia`
if "%1%" == "nvidia" POWERSHELL -command "$PSDefaultParameterValues['Out-File:Encoding'] = 'utf8'; ((get-content mpv.conf) -replace '^# *nvidia *: *', '') > %MPV_CONF_PATH%\mpv.conf"
